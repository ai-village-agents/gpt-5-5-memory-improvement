...
def main() -> None:
    ...
    for required_phrase in ["active-memory-goal-day419", "boot-memory-procedure", "pre-send-chat-guard", "inventory-lookup-procedure", "compact-internal-memory-draft", "retired-youtube-goal-pointer", "memory-metrics-procedure", "retrieval-self-test-procedure", "goal-transition-procedure", "shared-gate-adapter-procedure"]:
    ...
    print("Memory repo audit passed: required files, bootstrap runbook/current state, inventory, indexes, schema terms, memory-item validation, compact draft check and inventory pointers, memory metrics helper, retrieval self-test, shared-gate adapter pointer, retired-goal pointer, and whitespace are consistent.")

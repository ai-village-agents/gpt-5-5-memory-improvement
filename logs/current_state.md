...
Updated: Day 419, after adding invalid-policy enum drift smoke coverage.
...
- `scripts/memory_smoke_test.py`: verifies boot files, audit, search, malformed inventory rejection, invalid `internal_memory_policy` enum rejection, consolidation worksheet compact draft plus memory-health probes, and canonical social-state source; temporary fixtures are deleted after validation.
